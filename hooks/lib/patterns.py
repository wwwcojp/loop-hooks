...
- `*` は `/` を跨がない。`**` は直後が `/` か末尾で、それより前に glob 文字が無いか直前が
  `/` のとき跨ぐ(git と同じ)。
- 末尾以外にスラッシュが無ければ任意の深さの basename に一致。あればルート基準。末尾 `/` は
  ディレクトリ指定(配下が必要)。先頭 `!` は否定(後勝ち)、`\\!` はリテラル。不正はリテラル扱い。
"""
    ...
    while i < n:
        ...
        if c == "*":
            ...
            if j == -1:
                ...
                cls = body[i + 1 : j]
                if cls.startswith("!"):
                    cls = "^" + cls[1:] + "/"  # git と同じくクラスは / に一致しない(正は先読み)
                neg = cls[0] == "^"
                out.append(("" if neg else "(?!/)") + "[" + cls.replace("\\", "\\\\") + "]")
                i = j + 1
        elif c == "\\" and i + 1 < n:
