...
6. エージェントを閉じ込めない。同じフィンガープリントは同じエージェントについて続けて2度ブロックしない
...
- `*` と `?` は `/` を跨がない。`**` は直後が `/` か末尾で、それより前に glob 文字が無ければ
  跨ぐ(`**/x`、`x/**`、`x**/y`)。
- スラッシュを含まないパターン(`*.md`、`node_modules`)は任意の深さに一致。含むもの(`docs/*`、
  `src/**/*.py`)はリポジトリルート基準。
- 末尾の `/` はディレクトリ指定で、その配下すべてに一致する(`node_modules/`)。末尾のスラッシュだけでは
  ルート基準にならない(`node_modules/` は任意の深さに一致)。
- 不正なパターン(閉じない `[`、`[z-a]` のような逆順の範囲)はリテラルとして照合する(git は何にも一致させない)。
...
- **無限ループにはならない。** 意図的にフェイルオープンとして設計している。同じ
  フィンガープリントは同じエージェントについて2回続けてブロックしない(変更が無い＝修正を試みていない)。再入時の
...
`verified` は最後にゲートを通った時点のフィンガープリントで、worktree 内のすべてのセッションが
共有する(同じファイル、同じ判定のため)。`blocked` はスコープ——フィードバックを受け取った
セッション、サブエージェント(`session/agent_id`)、teammate(`session/teammate_name`)——を
ブロックされた時点のフィンガープリントに対応付けたもので、同じエージェントは同じ状態で
2度ブロックされない一方、他のエージェントはそれぞれ一度はフィードバックを受け取る。
ゲートが通るたびにクリアされ、64 スコープを上限とする。削除すれば次のターンで必ず
ゲートが走る。
...
- Claude Code は Stop フックの block が 8 回続くと自分でターンを終える。ゲートは同じ指紋の
  2 回目の失敗を警告で通すので、この上限には通常届かない。
