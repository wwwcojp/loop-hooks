...
- **無限ループにはならない。** 意図的にフェイルオープンとして設計している。再入時の
  2回目の失敗は警告に変わり、Claude Code 側でも連続ブロックは8回で打ち切られ、
  `TeammateIdle` では同じ状態を2回続けてブロックしない。いずれの場合もフィンガープリントは
  記録されないので、次に変更があればゲートは再び実行される。
